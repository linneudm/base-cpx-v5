	...
	if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not LocalPlayer["state"]["Prison"] and not Dynamic and not IsPauseMenuActive() and GetEntityHealth(Ped) > 100 then
		...
		if parseInt(#Clothes) > 0 then
			for Index, v in pairs(Clothes) do
				...
				exports["dynamic"]:AddButton("Aplicar", "Vestir-se com as vestimentas.", "dynamic:Clothes", "Apply-" .. v, Index,
					true)
				exports["dynamic"]:AddButton("Remover", "Deletar a vestimenta do armário.", "dynamic:Clothes", "Delete-" .. v,
					Index, true, true)
			end
		end
		...
		exports["dynamic"]:AddButton("Acessórios", "Colocar/Retirar os acessórios.", "player:Outfit", "Accessory", "clothes",
			true)
		...
		if IsEntityAVehicle(Vehicle) then
			if not IsPedInAnyVehicle(Ped) then
				if vRP.ClosestPed(3) then
					...
					exports["dynamic"]:AddButton("Colocar no Veículo", "Colocar no veículo mais próximo.", "player:cvFunctions",
						"cv", "closestpeds", true)
					exports["dynamic"]:AddButton("Remover do Veículo", "Remover do veículo mais próximo.", "player:cvFunctions",
						"rv", "closestpeds", true)
				end
			else
				exports["dynamic"]:AddMenu("Veículo", "Funções do veículo.", "vehicle")
				exports["dynamic"]:AddButton("Sentar no Motorista", "Sentar no banco do motorista.", "player:seatPlayer", "0",
					"vehicle", false)
				exports["dynamic"]:AddButton("Sentar no Passageiro", "Sentar no banco do passageiro.", "player:seatPlayer", "1",
					"vehicle", false)
				exports["dynamic"]:AddButton("Sentar em Outros", "Sentar no banco do passageiro.", "player:seatPlayer", "2",
					"vehicle", false)
				exports["dynamic"]:AddButton("Mexer nos Vidros", "Levantar/Abaixar os vidros.", "player:Windows", "", "vehicle",
					false)
			end
			...
			exports["dynamic"]:AddButton("Porta do Passageiro", "Abrir porta do passageiro.", "player:Doors", "2", "doors",
				true)
			exports["dynamic"]:AddButton("Porta Traseira Esquerda", "Abrir porta traseira esquerda.", "player:Doors", "3",
				"doors", true)
			exports["dynamic"]:AddButton("Porta Traseira Direita", "Abrir porta traseira direita.", "player:Doors", "4",
				"doors", true)
		...
		for Permission, v in pairs(Groups) do
			if not v.Block and LocalPlayer.state[Permission] then
				...
				local Event = (Permission == "LSPD" or Permission == "BCSO") and GetResourceState('mdt'):find('start') and
						"mdt:Open" or "painel:Opened"

				exports["dynamic"]:AddButton(v.Name or Permission, "Painel de Controle do usuário.", Event, Permission, "painel",
					false)
		...
		exports["dynamic"]:AddButton("Estatísticas da Cidade", "Tudo sobre nossa cidade.", "admin:Dynamic", "stats", "others",
			true)
		exports["dynamic"]:AddButton("Propriedades", "Marcar/Desmarcar propriedades no mapa.", "propertys:Blips", "",
			"others", false)
		exports["dynamic"]:AddButton("Ferimentos", "Verificar ferimentos no corpo.", "paramedic:Injuries", "", "others",
			false)
		...
		if Stats then
			...
			for Name, Points in pairs(Stats) do
				exports["dynamic"]:AddButton(Name, "Você possuí um total de <rare>" .. Points .. " " .. Name .. "</rare>.", "",
					"", "Stats", false)
			end
		end
...
RegisterCommand("EmergencyFunctions", function()
	if not IsPauseMenuActive() and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not LocalPlayer["state"]["Prison"] and not Dynamic then
		...
		if CheckPolice() then
			...
			exports["dynamic"]:AddButton("Serviço", "Finalizar expediente de trabalho.", "dynamic:ExitService", "Policia",
				false, true)

			exports["dynamic"]:AddButton("Companheiros", "Verifique seus companheiros em serviço.", "admin:Dynamic",
				"statsPolicia", false, true)
			...
			if Health > 100 and not IsPedInAnyVehicle(Ped) then
				...
				exports["dynamic"]:AddButton("Colocar no Veículo", "Colocar no veículo mais próximo.", "player:cvFunctions", "cv",
					"player", true)
				exports["dynamic"]:AddButton("Remover do Veículo", "Remover do veículo mais próximo.", "player:cvFunctions", "rv",
					"player", true)
				exports["dynamic"]:AddButton("Remover Chapéu", "Remover da pessoa mais próxima.", "skinshop:Remove", "Hat",
					"player", true)
				exports["dynamic"]:AddButton("Remover Máscara", "Remover da pessoa mais próxima.", "skinshop:Remove", "Mask",
					"player", true)
				exports["dynamic"]:AddButton("Remover Óculos", "Remover da pessoa mais próxima.", "skinshop:Remove", "Glasses",
					"player", true)
		...
		elseif LocalPlayer["state"]["Paramedico"] then
			if Health > 100 and not IsPedInAnyVehicle(Ped) then
				exports["dynamic"]:AddButton("Serviço", "Finalizar expediente de trabalho.", "dynamic:Service", "Paramedico",
					false, true)

				exports["dynamic"]:AddButton("Companheiros", "Verifique seus companheiros em serviço.", "admin:Dynamic",
					"statsParamedico", false, true)
				...
				exports["dynamic"]:AddButton("Medical Center", "Fardamento de paramédico.", "player:Preset", "6", "preMedic",
					true)
				exports["dynamic"]:AddButton("Medical Center", "Fardamento de paramédico interno.", "player:Preset", "7",
					"preMedic", true)
		...
		elseif LocalPlayer["state"]["Mecanico"] then
			exports["dynamic"]:AddButton("Serviço", "Finalizar expediente de trabalho.", "dynamic:ExitService", "Mecanico",
				false, true)
			exports["dynamic"]:Open()
		else
			...
			for Permission, v in pairs(Services) do
				if #(Coords - v["Coords"]) <= v["Distance"] then
					exports["dynamic"]:AddButton("Serviço", "Iniciar expediente de trabalho.", "dynamic:Service", v["Permission"],
						false, true)
					exports["dynamic"]:Open()
		...
		if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not Dynamic and not LocalPlayer["state"]["Bennys"] and MumbleIsConnected() then
			if GlobalState["Commands"] then
				TriggerEvent("Notify", "default",
					"O menu <b>Insert</b> está desativado pois os comandos via <b>Chat</b> estão habilitados.", false, 10000)
			else
				...
				exports["dynamic"]:AddButton("Armour", "Deixar o ID com tudo Colete 100%.", "admin:Dynamic", "armour", "admin",
					true)
				exports["dynamic"]:AddButton("Commands", "Ativar/Desativar os Comandos por Chat.", "admin:Dynamic", "commands",
					"admin", true)
				...
				exports["dynamic"]:AddButton("Chat Announce", "Enviar anúncio no chat para todos.", "admin:Dynamic",
					"chatannounce", "admin", true)
				exports["dynamic"]:AddButton("Clear Prison", "Limpar prisão do ID.", "admin:Dynamic", "clearprison", "admin",
					true)
				...
				exports["dynamic"]:AddButton("Change Weather", "Mudar o Clima do jogo.", "admin:Dynamic", "weatherset", "sky",
					true)
				...
				exports["dynamic"]:AddButton("Remove Group", "Remover o grupo de um ID.", "admin:Dynamic", "ungroup", "groups",
					true)
				...
				exports["dynamic"]:AddButton("Nitro", "Defina o Nitro no veículo atual.", "admin:Dynamic", "nitro", "vehicles",
					true)
				exports["dynamic"]:AddButton("Fuel", "Defina a Gasolina no veículo atual.", "admin:Dynamic", "fuel", "vehicles",
					true)
				exports["dynamic"]:AddButton("Lockpick", "Desbloqueia o veículo atual.", "admin:Dynamic", "lockpick", "vehicles",
					true)
				exports["dynamic"]:AddButton("Hash", "Pegar a Hash do veículo atual.", "admin:Dynamic", "hash", "vehicles", true)
				exports["dynamic"]:AddButton("Give Car", "Envie um veículo para o ID.", "admin:Dynamic", "setcar", "vehicles",
					true)
				exports["dynamic"]:AddButton("Remove Car", "Remove um veículo do ID.", "admin:Dynamic", "remcar", "vehicles",
					true)
				...
				exports["dynamic"]:AddButton("" .. ItemName("gemstone") .. "s.", "Inserir " .. ItemName("gemstone") .. "s no ID.",
					"admin:Dynamic", "gem", "wallet", true)
				...
				exports["dynamic"]:AddButton("Item All", "Dar Itens para todos conectados.", "admin:Dynamic", "itemall", "item",
					true)
				...
				exports["dynamic"]:AddButton("Debug", "Ativar/Desativar o modo Desenvolvedor.", "admin:Dynamic", "debug", "basic",
					true)
				...
				exports["dynamic"]:AddButton("Tpcds", "Teletransporte para uma coordenada.", "admin:Dynamic", "tpcds", "basic",
					true)
				exports["dynamic"]:AddButton("Tptome", "Teletransporte um ID para você.", "admin:Dynamic", "tptome", "basic",
					true)
				exports["dynamic"]:AddButton("Tpto", "Teletransporte para um ID.", "admin:Dynamic", "tpto", "basic", true)
				exports["dynamic"]:AddButton("Tpway", "Teletransporte para uma marcação no GPS.", "admin:Dynamic", "tpway",
					"basic", true)
				exports["dynamic"]:AddButton("Limpar Área", "Limpar a área próxima a você.", "admin:Dynamic", "limparea", "basic",
					true)
				exports["dynamic"]:AddButton("Players", "Verifique quantos onlines existem.", "admin:Dynamic", "players", "basic",
					true)
				exports["dynamic"]:AddButton("Players Conectados", "Verifique os players onlines.", "admin:Dynamic",
					"playersconnected", "basic", true)

				exports["dynamic"]:AddMenu("Divertidos", "Clique para mais informações.", "fun")
				exports["dynamic"]:AddButton("Magneto", "Ativar/Desativar o efeito Magneto.", "admin:Dynamic", "magneto", "fun",
					true)
				exports["dynamic"]:AddButton("Flash", "Ativar/Desativar o efeito Flash.", "admin:Dynamic", "flash", "fun", true)
				exports["dynamic"]:AddButton("Vehicle Speed", "Mude a velocidade do veículo.", "admin:Dynamic", "vehiclespeed",
					"fun", true)
				exports["dynamic"]:AddButton("Tyre Burst", "Exploda o pneu do veículo do ID.", "admin:Dynamic", "tyreburst",
					"fun", true)
				exports["dynamic"]:AddButton("Create Lightning", "Crie diversos raios pela cidade.", "admin:Dynamic",
					"createlightning", "fun", true)
