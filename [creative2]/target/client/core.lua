...
AddEventHandler("onClientResourceStart", function(Resource)
	...
	AddTargetModel(
		{ -832573324, -1430839454, 1457690978, 1682622302, 402729631, -664053099, 1794449327, 307287994, -1323586730, 111281960, -541762431, -745300483, -417505688 },
		{
			options = {
				{
	...
	AddTargetModel(
		{ 684586828, 577432224, -1587184881, -1426008804, -228596739, 1437508529, -1096777189, -468629664, 1143474856, -2096124444, -115771139, 1329570871, -130812911 },
		{
			options = {
				{
...
function TargetEnable()
	...
	TriggerEvent("target:ParamedicRefreshLabels")

	local TargetAltPrintLast = 0
	local function PrintTargetAimIfAlt(HitCoords, EntityHit, Entitys)
		...
		if not (IsControlPressed(0, 19) or IsDisabledControlPressed(0, 19)) then
			return
		end

		local Now = GetGameTimer()
		if Now - TargetAltPrintLast < 200 then
			return
		end

		TargetAltPrintLast = Now

		local X = HitCoords["x"] or HitCoords.x
		local Y = HitCoords["y"] or HitCoords.y
		local Z = HitCoords["z"] or HitCoords.z
		local EntityType = "n/a"
		if EntityHit and Entitys and DoesEntityExist(Entitys) then
			EntityType = tostring(GetEntityType(Entitys))
		end

		print(("[target] Alt+mirando: x=%.4f y=%.4f z=%.4f | rayHit=%s | entityType=%s"):format(X, Y, Z, tostring(EntityHit),
			EntityType))
	end
	...
	while Actived do
		...
		PrintTargetAimIfAlt(HitCoords, EntityHit, Entitys)
		...
		for Index, v in pairs(Zones) do
			...
			if Zones[Index]:isPointInside(HitCoords) and #(Coords - Zones[Index]["center"]) <= v["targetoptions"]["Distance"] then
				...
				while Sucess do
					...
					local Ped = PlayerPedId()
					local OtherCoords, OtherEntity, OtherHit = RayCastGamePlayCamera()
					PrintTargetAimIfAlt(OtherCoords, OtherHit, OtherEntity)
		...
		if EntityHit and GetEntityType(Entitys) ~= 0 then
			...
			if IsEntityAVehicle(Entitys) and GetEntityHealth(Ped) > 100 and #(Coords - HitCoords) <= 1.0 then
				...
				Selected = { GetVehicleNumberPlateText(Entitys), GetEntityArchetypeName(Entitys), Entitys, Network,
					GetEntityModel(Entitys), false }
