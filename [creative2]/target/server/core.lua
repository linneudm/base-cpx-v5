	...
	if Passport then
		if vRP.GetHealth(source) <= 100 then
			if vRP.TakeItem(Passport, "paramedicpass", 1, true) then
				...
				TriggerEvent("hud:Reposed", source, Passport, 900)
		...
		if os.time() >= Announces[Service] then
			if Service == "Policia" then
				if vRP.HasGroup(Passport, "Policia") then
					...
					TriggerClientEvent("Notify", -1, "policia",
						"Sinta-se seguro(a) agora mesmo, pois há policiais realizando patrulhas em nossa cidade neste exato instante.",
						"Los Santos Police Department", 15000)
				else
			...
			elseif Service == "Paramedico" then
				if vRP.HasGroup(Passport, "Paramedico") then
					...
					TriggerClientEvent("Notify", -1, "hospital",
						"Estamos em busca de doadores de sangue, seja solidário e ajude o próximo, procure um de nossos profissionais.",
						"Pillbox Medical Center", 15000)
				else
					TriggerClientEvent("Notify", source, "vermelho", "Você não pode enviar um anúncio.", "Aviso", 5000)
				end
			elseif Service == "Mecanico" then
				if vRP.HasGroup(Passport, "Mecanico") then
					Announces[Service] = os.time() + 600
					TriggerClientEvent("Notify", -1, "mecanico",
						"Necessita dos serviços de uma oficina mecânica? Temos profissionais disponíveis. Faça agora mesmo a sua solicitação.",
						"Los Santos Customs", 15000)
				else
					TriggerClientEvent("Notify", source, "vermelho", "Você não pode enviar um anúncio.", "Aviso", 5000)
				end
			else
				if vRP.HasGroup(Passport, Service) then
					Announces[Service] = os.time() + 600
					TriggerClientEvent("Notify", -1, "default",
						"Estamos em busca de trabalhadores, compareça ao estabelecimento, procure um de nossos funcionários e consulte nosso serviço de entregas.",
						Service, 15000)
				else
					TriggerClientEvent("Notify", source, "vermelho", "Você não pode enviar um anúncio.", "Aviso", 5000)
				end
