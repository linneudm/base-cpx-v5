...
function Creative.Player()
    ...
    local Permissions = Config.OtherPermissions[Permission] and Config.OtherPermissions[Permission][Hierarchy] or
    Config.Permissions
    ...
    local Departments = { { Max = vRP.Permissions(Permission, 'Members'), Name = Name, Hierarchy = vRP.Hierarchy(Permission) }, { Level = Hierarchy, Passport = Passport, Name = vRP.FullName(Passport), Avatar = Avatar },
        Permissions }
...
function Creative.User(Target)
    ...
    local Records = exports['oxmysql']:query_async(
    'SELECT id AS Id, Timestamp AS Date, Doctor FROM painel_creative_paramedic WHERE Passport = ? ORDER BY Timestamp DESC',
        { Target })
    ...
    return { { Passport = Target, Name = vRP.FullName(Target), Phone = vRP.Phone(Target) or "N/A", Gender = vRP.Identity(Target).Sex or "N/A", Blood = Sanguine(vRP.Identity(Target).Blood) or "N/A", MedicPlan = MedicPlan and MedicPlan.Medic ~= 0 or false, Avatar = Avatar },
        Records }
end
...
function Creative.Avatar(Target, Image)
    ...
    if Avatar then
        ...
        'UPDATE avatars SET Image = ?, Permission = ? WHERE Passport = ?', { Image, Permission, Target })
        ...
        'INSERT INTO avatars (Passport, Image, Permission) VALUES (?, ?, ?)', { Target, Image, Permission })
...
function Creative.MedicPlan(Passport)
    ...
    'UPDATE characters SET Medic = CASE WHEN Medic IS NULL OR Medic = 0 THEN UNIX_TIMESTAMP() ELSE 0 END WHERE id = ?',
        { Passport })
    return true
end
...
function Creative.GetMedicalRecord(Id)
    local Records = exports['oxmysql']:single_async(
    "SELECT p.id AS Id, p.Timestamp AS Date, p.Description, CONCAT(d.Name, ' ', d.Lastname) AS Doctor, CONCAT(c.Name, ' ', c.Lastname) AS Patient FROM painel_creative_paramedic p LEFT JOIN characters d ON d.id = p.Doctor LEFT JOIN characters c ON c.id = p.Passport WHERE p.id = ?",
        { Id })
    return Records
end
...
function Creative.CreateMedicalRecord(Data)
    ...
    'INSERT INTO painel_creative_paramedic (Passport, Doctor, Permission, Timestamp, Description) VALUES (?, ?, ?, UNIX_TIMESTAMP(), ?)',
        { Data.Passport, Passport, Permission, Description })
    return true
end
...
function Creative.UpdateMedicalRecord(Data)
    ...
    'UPDATE painel_creative_paramedic SET Description = ?, Timestamp = UNIX_TIMESTAMP() WHERE id = ?',
...
function Creative.SearchUser(Search)
    ...
    if Permission == "Paramedico" then
        ...
        'SELECT id AS Passport, Name, Lastname, Medic FROM characters WHERE Deleted = 0 AND ( LOWER(Name) LIKE ? OR LOWER(Lastname) LIKE ? OR CAST(id AS CHAR) = ? ) LIMIT 50',
            { Lookup, Lookup, Search })

        for _, User in ipairs(Users) do
            table.insert(Results,
                { Passport = User.Passport, Name = User.Name .. ' ' .. User.Lastname, MedicPlan = User.Medic ~= 0 })
        end
...
function Creative.Members()
    ...
    'SELECT Image, Name, Members FROM painel_creative_tags WHERE Permission = ?', { Permission }) or {}
    ...
    for Target in pairs(Groups) do
        ...
        if Identity then
            ...
            table.insert(Members,
                { Passport = Target, Name = vRP.FullName(Target), Hierarchy = Hierarchy, Tags = Assigned, Service =
                Service[Target] and 1 or 0, Hours = vRP.Playing(Target, Permission) or 0, Status = vRP.Source(Target) and
                "Ativo a " .. Calculated or "Inativo a " .. Calculated })
        end
    end
...
function Creative.Tags()
    ...
    local Consult = exports['oxmysql']:query_async('SELECT * FROM painel_creative_tags WHERE Permission = ?',
        { Permission }) or {}
    if Consult[1] then
        for i = 1, #Consult do
...
function Creative.GetTag(Data)
    local Consult = exports['oxmysql']:single_async('SELECT Id, Name, Members FROM painel_creative_tags WHERE Id = ?',
        { Data.Id })
...
function Creative.CreateTag(Data)
    ...
    local Consult = exports['oxmysql']:scalar_async(
    'SELECT COUNT(*) as Total FROM painel_creative_tags WHERE Permission = ?', { Permission })

    if Consult >= Config.MaxAmountTags then
        TriggerClientEvent("painel:Notify", source, "Erro",
            "O limite de <b class=\"text-white\">" .. Config.MaxAmountTags .. " tags</b> foi atingido.", "vermelho")
        return false
    end

    local Consult = exports['oxmysql']:insert_async(
    'INSERT INTO painel_creative_tags (Name, Image, Permission) VALUES (@Name, @Image, @Permission)',
        { Name = Data.Name, Image = Data.Image, Permission = Permission })

    if Consult then
        TriggerClientEvent("painel:Notify", source, "Sucesso",
            "A tag <b class=\"text-white\">" .. Data.Name .. "</b> foi criada com sucesso.", "verde")
    else
        TriggerClientEvent("painel:Notify", source, "Erro",
            "Falha ao criar a tag <b class=\"text-white\">" .. Data.Name .. "</b>.", "vermelho")
        return false
    end
...
function Creative.UpdateTag(Data)
    ...
    local Consult = exports['oxmysql']:execute_async(
    'UPDATE painel_creative_tags SET Name = ?, Image = ? WHERE Id = ? AND Permission = ?',
        { Data.Name, Data.Image, Data.Id, Permission })

    if Consult then
        TriggerClientEvent("painel:Notify", source, "Sucesso",
            "A tag <b class=\"text-white\">" .. Data.Name .. "</b> foi atualizada com sucesso.", "verde")
        return true
    else
        TriggerClientEvent("painel:Notify", source, "Erro",
            "Falha ao atualizar a tag <b class=\"text-white\">" .. Data.Name .. "</b>.", "vermelho")
        return false
    end
end
...
function Creative.AssignTag(Data)
    ...
    local Consult = exports['oxmysql']:single_async('SELECT Name, Members FROM painel_creative_tags WHERE Id = ?',
        { Data.Id })

    local Members = json.decode(Consult.Members) or {}
    for _, Member in ipairs(Members) do
        if Member == Data.Passport then
            TriggerClientEvent("painel:Notify", source, "Error",
                "O passaporte <b class=\"text-white\">" ..
                Data.Passport .. "</b> já possui a tag <b class=\"text-white\">" .. Consult.Name .. "</b>.", "vermelho")
            return false
        end
    end
    ...
    exports['oxmysql']:execute_async('UPDATE painel_creative_tags SET Members = ? WHERE Id = ?',
        { json.encode(Members), Data.Id })

    TriggerClientEvent("painel:Notify", source, "Sucesso",
        "O passaporte <b class=\"text-white\">" ..
        Data.Passport .. "</b> foi adicionado com sucesso à tag <b class=\"text-white\">" .. Consult.Name .. "</b>.",
        "verde")
...
function Creative.RemoveTag(Data)
    ...
    if Sucess then
        TriggerClientEvent("painel:Notify", source, "Sucesso",
            "O passaporte <b class=\"text-white\">" ..
            Passport .. "</b> foi removido com sucesso da tag <b class=\"text-white\">" .. Consult.Name .. "</b>.",
            "verde")
    else
        TriggerClientEvent("painel:Notify", source, "Erro",
            "Não foi possível localizar o passaporte <b class=\"text-white\">" ..
            Passport .. "</b> na tag <b class=\"text-white\">" .. Consult.Name .. "</b>", "vermelho")
    end
...
function Creative.DestroyTag(Data)
    ...
    local Consult = exports['oxmysql']:execute_async('DELETE FROM painel_creative_tags WHERE id = ? AND Permission = ?',
        { Data['Id'], Permission })
...
function Creative.Invite(Data)
    ...
    if GroupType == 'Work' and Identity.Groups >= os.time() then
        TriggerClientEvent("painel:Notify", source, "Atenção",
            "O passaporte escolhido não pode ser convidado para um grupo no momento.", "amarelo", 5000)
        return false
    end
    ...
    if not GroupType or GroupType ~= 'Work' or (GroupType == 'Work' and not vRP.GetUserType(Data.Passport, 'Work')) then
        ...
        if vRP.Request(vRP.Source(Data.Passport), "Grupos", "Você foi convidado(a) para participar do grupo <b class=\"text-white\">" .. Permission .. "</b>, gostaria de estar entrando do mesmo?") then
            ...
            return { Passport = Data.Passport, Name = vRP.FullName(Passport), Hierarchy = vRP.HasPermission(
            Data.Passport, Permission), Service = 1 }
        else
...
function Creative.Hierarchy(Data)
    ...
    if Data.Mode:find('Promote') or Data.Mode:find('Demote') then
        ...
        return { Passport = Data.Passport, Name = vRP.FullName(Data.Passport), Hierarchy = vRP.HasPermission(
        Data.Passport, Permission), Service = vRP.Source(Data.Passport) and 1 or 0, }
    end
...
function Creative.Announcements()
    ...
    local Permission = Permission[Passport]
    local Consult = exports['oxmysql']:query_async(
    'SELECT * FROM painel_creative_announcements WHERE Permission = @Permission', { Permission = Permission })
    return Consult
end
...
function Creative.CreateAnnouncement(Data)
    ...
    local Consult = exports['oxmysql']:scalar_async(
    'SELECT COUNT(*) FROM painel_creative_announcements WHERE Permission = ?', { Permission })

    if Consult >= Config.MaxAmountAnnouncements then
        TriggerClientEvent("painel:Notify", source, "Erro",
            "O limite de <b class=\'text-white\">" .. Config.MaxAmountAnnouncements .. " avisos</b> foi atingido.",
            "vermelho")
        return false
    end
    ...
    local Consult = exports['oxmysql']:insert_async(
    'INSERT INTO painel_creative_announcements (Title, Description, Date, Permission) VALUES (?, ?, ?, ?)',
        { Data.Title, Description, os.time(), Permission })
    ...
    if Consult then
        ...
        for _, Target in pairs(Groups) do
            if Target ~= source then
                TriggerClientEvent("Notify", Target, Name, "<b class=\'text-white\">" ..
                Data.Title .. "</b>: " .. Description, "amarelo")
            end
        end
...
function Creative.UpdateAnnouncement(Data)
    ...
    local Consult = exports['oxmysql']:execute_async(
    'UPDATE painel_creative_announcements SET Title = ?, Description = ?, Updated = ? WHERE id = ? AND Permission = ?',
        { Data.Title, Description, os.time(), Data.Id, Permission })
...
function Creative.DestroyAnnouncement(Data)
    ...
    local Consult = exports['oxmysql']:execute_async(
    'DELETE FROM painel_creative_announcements WHERE id = ? AND Permission = ?', { Data.Id, Permission })
...
function Creative.Perks()
    ...
    for k, v in ipairs(Config.Perks) do
        ...
        Data[#Data + 1] = { Active = (v.Type == "Premium") and vRP.Permissions(Permission, "Premium") or false, Description =
        v.Description, Image = v.Image, Price = Price, Title = v.Title }
    end
...
function Creative.PerksBuy(Id)
    ...
    local Price = type(Perks.Price) == "table" and Perks.Price[vRP.Permissions(Permission, "Members") + 1] or Perks
    .Price
    ...
    exports['oxmysql']:insert_async(
    'INSERT INTO painel_creative_transactions (Type, Passport, Value, Date, Permission) VALUES (?, ?, ?, ?, ?)',
        { "Perks", Passport, Price, os.time(), Permission })

    TriggerClientEvent("painel:Notify", source, "Sucesso",
        "Vantagem <b class=\"text-white\">" ..
        Perks.Title .. "</b> comprada com sucesso por <b class=\"text-white\">$" .. Price .. "</b>.", "verde")
    return true
end
...
function Creative.Bank()
    ...
    local Consult = exports['oxmysql']:query_async(
    'SELECT * FROM painel_creative_transactions WHERE Permission = @Permission LIMIT 10', { Permission = Permission })
    ...
    for _, Data in ipairs(Consult) do
        table.insert(Transactions,
            ...
            Data.Type, Value = Data.Value, Date = Data.Date })
    end
...
function Creative.DepositBank(Data)
    ...
    if vRP.PaymentBank(Passport, Data.Value, true) then
        exports['oxmysql']:insert_async(
        'INSERT INTO painel_creative_transactions (Type, Passport, Value, Date, Permission) VALUES (\'Deposit\', ?, ?, ?, ?)',
            { Passport, Data.Value, os.time(), Permission })

        vRP.PermissionsUpdate(Permission, "Bank", "+", Data.Value)

        TriggerClientEvent("painel:Notify", source, "Sucesso",
            "Depósito de <b class=\"text-white\">$" .. Data.Value .. "</b> realizado com sucesso.", "verde")
        return true
    else
...
function Creative.WithdrawBank(Data)
    ...
    exports['oxmysql']:insert_async(
    'INSERT INTO painel_creative_transactions (Type, Passport, Value, Date, Permission) VALUES (\'Withdraw\', ?, ?, ?, ?)',
        { Passport, Data.Value, os.time(), Permission })
    ...
    TriggerClientEvent("painel:Notify", source, "Sucesso",
        "Saque de <b class=\"text-white\">$" ..
        Data.Value .. "</b> realizado com uma taxa de <b class=\"text-white\">$" .. Tax .. "</b> aplicada.", "verde")
...
function Creative.TransferBank(Data)
    ...
    exports['oxmysql']:insert_async(
    'INSERT INTO painel_creative_transactions (Type, Passport, Value, Transfer, Date, Permission) VALUES (\'Transfer\', ?, ?, ?, ?, ?)',
        { Passport, Data.Value, Data.Passport, os.time(), Permission })
    ...
    TriggerClientEvent("painel:Notify", source, "Sucesso",
        "Transferência de <b class=\"text-white\">$" ..
        Data.Value ..
        "</b> para o passaporte <b class=\"text-white\">" ..
        Data.Passport .. "</b> realizada com uma taxa de <b class=\"text-white\">$" .. Tax .. "</b> aplicada.", "verde")
    ...
    if Source then
        TriggerClientEvent("Notify", Source, "Sucesso",
            "<b class=\"text-white\">" ..
            Title .. "</b> fez uma transferência de <b class=\"text-white\">$" .. Data.Value .. "</b> para você.",
            "verde", 10000)
    end
