...
-----------------------------------------------------------------------------------------------------------------------------------------
GlobalState["Plates"] = {}
if type(GlobalState["Nitro"]) ~= "table" then
	GlobalState["Nitro"] = {}
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- PREMIUM
...
function Hensa.Check(Number)
	...
	if Passport and not exports["hud"]:Repose(Passport, source) and not exports["hud"]:Wanted(Passport, source) then
		return true
	end
	...
	if BankVerification then
		if exports["bank"]:CheckFines(Passport) or exports["bank"]:CheckTaxs(Passport) then
			TriggerClientEvent("Notify", source, "Atenção",
				"Você está temporariamente proibido de utilizar esse sistema por pendências com o <b>Banco</b>.", "amarelo", 5000)
			return false
		end
	end
		...
		if vRP.HasGroup(Passport, "Emergencia") then
			...
			if vRP.Request(source, "Garagem", "Guardar um veículo por <b>$" .. Price .. "</b> dólares?") then
				if vRP.PaymentFull(Passport, Price) then
					...
					TriggerClientEvent("Notify", source, "Aviso", "<b>" .. ItemName(DefaultMoneyOne) .. "</b> insuficientes.",
						"vermelho", 5000)
				end
			end
		end
			...
			if Consult[1] then
				if parseInt(Consult[1]["Passport"]) == Passport or vRP.InventoryFull(Passport, "propertys-" .. Consult[1]["Serial"]) then
					if os.time() > Consult[1]["Tax"] then
						TriggerClientEvent("Notify", source, "Atenção", "Aluguel atrasado, procure um <b>Corretor de Imóveis</b>.",
							"amarelo", 5000)
						return false
					end
				else
			...
			for _, v in pairs(Consult) do
				if VehicleExist(v["Vehicle"]) then
					if v["Mode"] == "dismantle" and v["Dismantle"] <= os.time() then
						...
						TriggerClientEvent("Notify", source, "Jimmy Jango",
							"Estamos recuperando o veículo <b>" ..
							VehicleName(v["Vehicle"]) .. "</b>, espere um pouco para abrir a garagem.", "azul", 10000)
						return false
					end
...
AddEventHandler("garages:Impound", function(Name)
	...
	if Passport then
		...
		local Message = "A liberação do veículo <b>" ..
		VehicleName(Name) ..
		"</b> tem o custo de <b>$" .. Dotted(VehiclePrice) .. " dólares</b>, deseja prosseguir com a liberação do mesmo?"
	...
	if Passport then
		...
		if Consult[1] then
			if Consult[1]["Tax"] <= os.time() then
				...
				local Message = "As taxas do veículo <b>" ..
				VehicleName(Name) ..
				"</b> estão em <b>$" ..
				Dotted(Price) .. " " .. ItemName(DefaultMoneyOne) .. "</b>, deseja prosseguir com a liberação do mesmo?"
				if vRP.Request(source, "Garagem", Message) then
					if vRP.PaymentFull(Passport, Price) then
	...
	if Passport then
		...
		if Consult[1] then
			if Consult[1]["Mode"] == "dismantle" then
				TriggerClientEvent("Notify", source, "Atenção",
					"Jimmy Jango contratou você para o resgate, você não pode vender o veículo dele.", "amarelo", 5000)
				return
			end
			...
			local Message = "Vender o veículo <b>" ..
			VehicleName(Name) .. "</b> por <b>$" .. Dotted(Price) .. " " .. ItemName(DefaultMoneyOne) .. "</b>?"
			if vRP.Request(source, "Garagem", Message) then
				local Consult = vRP.Query("vehicles/selectVehicles", { Passport = Passport, Vehicle = Name })
				if Consult[1] then
	...
	if Passport then
		...
		if TheVehicle[1] then
			if TheVehicle[1]["Mode"] == "dismantle" then
				TriggerClientEvent("Notify", source, "Atenção",
					"Jimmy Jango contratou você para o resgate, você não pode transferir o veículo dele.", "amarelo", 5000)
				return
			end
				...
				if Identity and OtherPassport then
					if OtherPassport == Passport then
						...
						if vRP.Request(source, "Garagem", "Transferir o veículo <b>" .. VehicleName(Name) .. "</b> para <b>" .. Identity["Name"] .. " " .. Identity["Lastname"] .. "</b>?") then
							...
							if Vehicle[1] then
								TriggerClientEvent("Notify", source, "Atenção",
									"<b>" .. Identity["Name"] .. " " .. Identity["Lastname"] .. "</b> já possui este modelo de veículo.",
									"amarelo", 5000)
							else
								...
								local Datatable = vRP.Query("entitydata/GetData",
									{ Name = "LsCustoms:" .. Passport .. ":" .. Name .. ":" .. TheVehicle[1]["id"] })
								if parseInt(#Datatable) > 0 then
									vRP.Query("entitydata/SetData",
										{ Name = "LsCustoms:" .. OtherPassport .. ":" .. Name .. ":" .. TheVehicle[1]["id"], Information =
										Datatable[1]["Information"] })
									vRP.Query("entitydata/RemoveData",
										{ Name = "LsCustoms:" .. Passport .. ":" .. Name .. ":" .. TheVehicle[1]["id"] })
								end
		...
		if not vehicle[1] then
			if parseInt(Gemstone) > 0 then
				...
				if vRP.Request(source, "Garagem", "Alugar o veículo <b>" .. VehicleName(Name) .. "</b> por <b>" .. Gemstone .. "</b> " .. ItemName(DefaultMoneySpecial) .. "?") then
					if vRP.PaymentGemstone(Passport, Gemstone) then
						...
						TriggerClientEvent("Notify", source, "Sucesso",
							"Aluguel do veículo <b>" .. VehicleName(Name) .. "</b> concluído.", "verde", 5000)
						vehicle = vRP.Query("vehicles/selectVehicles", { Passport = Passport, Vehicle = Name })
					else
						TriggerClientEvent("Notify", source, "Aviso", "<b>" .. ItemName(DefaultMoneySpecial) .. "</b> insuficientes.",
							"vermelho", 5000)
						return
					end
				else
				...
				if parseInt(VehiclePrice) > 0 then
					local Message = "Comprar <b>" ..
					VehicleName(Name) .. "</b> por <b>$" .. Dotted(VehiclePrice) .. " " .. ItemName(DefaultMoneyOne) .. "</b>?"
					if vRP.Request(source, "Garagem", Message) then
						if vRP.PaymentFull(Passport, VehiclePrice) then
		...
		if vehicle[1] then
			...
			if Spawn[Plate] then
				if not Signal[Plate] then
					...
					if os.time() >= parseInt(Searched[Passport]) then
						...
						if DoesEntityExist(Network) and not IsPedAPlayer(Network) and GetEntityType(Network) == 2 then
							...
							TriggerClientEvent("Notify", source, "Garagem",
						...
						TriggerClientEvent("Notify", source, "Observação",
							"Rastreador só pode ser ativado a cada <b>60</b> segundos.", "azul", 5000)
					end
				else
				...
				elseif vehicle[1]["Arrest"] >= os.time() then
					TriggerClientEvent("Notify", source, "Atenção",
						"Veículo apreendido, dirija-se até o <b>Impound</b> e efetue o pagamento da liberação do mesmo.", "amarelo",
						10000)
					TriggerClientEvent("garages:Close", source)
				else
						...
						if vehicle[1]["Rental"] <= os.time() then
							...
							if vRP.Request(source, "Garagem", "Atualizar o aluguel do veículo <b>" .. VehicleName(Name) .. "</b> por <b>" .. Dotted(Gemstone) .. " " .. ItemName(DefaultMoneySpecial) .. "</b>?") then
								if vRP.PaymentGemstone(Passport, Gemstone) then
									...
									TriggerClientEvent("Notify", source, "Sucesso",
										"Aluguel do veículo <b>" .. VehicleName(Name) .. "</b> atualizado.", "verde", 5000)
								else
									TriggerClientEvent("Notify", source, "Aviso",
										"<b>" .. ItemName(DefaultMoneySpecial) .. "</b> insuficientes.", "vermelho", 5000)
									return
								end
							else
								return
					...
					if Coords then
						local LsCustoms = nil
						local Datatable = vRP.Query("entitydata/GetData",
							{ Name = "LsCustoms:" .. Passport .. ":" .. Name .. ":" .. vehicle[1]["id"] })
						if parseInt(#Datatable) > 0 then
						...
						if Garages[Number]["payment"] then
							if vRP.UserPremium(Passport) then
								...
								local Exist, Network = Hensa.ServerVehicle(Name, Coords[1], Coords[2], Coords[3], Coords[4], Plate,
									vehicle[1]["Nitro"], vehicle[1]["Doors"], vehicle[1]["Body"])

								if Exist then
									local Networked = NetworkGetEntityFromNetworkId(Network)

									vCLIENT.CreateVehicle(-1, Name, Network, vehicle[1]["Engine"], vehicle[1]["Health"], LsCustoms,
										vehicle[1]["Windows"], vehicle[1]["Tyres"], vehicle[1]["Brakes"], false)
									TriggerClientEvent("Notify", source, "Próximo pagamento", CompleteTimers(vehicle[1]["Tax"] - os.time()),
										"azul", 5000)
								...
								if vehicle[1]["Mode"] == "dismantle" then
									...
									if Exist then
										...
										vCLIENT.CreateVehicle(-1, Name, Network, vehicle[1]["Engine"], vehicle[1]["Health"], LsCustoms,
											vehicle[1]["Windows"], vehicle[1]["Tyres"], vehicle[1]["Brakes"], false)
										TriggerClientEvent("Notify", source, "Vencimento",
											CompleteTimers(vehicle[1]["Dismantle"] - os.time()), "azul", 5000)
										Entity(Networked)["state"]:set("Fuel", vehicle[1]["Fuel"], true)
										TriggerEvent("engine:InsertBrakes", Network, vehicle[1]["Brakes"])
										Spawn[Plate] = { Passport, Name, Network }
								...
								elseif vehicle[1]["Mode"] == "normal" then
									...
									if vRP.HasGroup(Passport, "Premium") then
										...
										if Exist then
											...
											vCLIENT.CreateVehicle(-1, Name, Network, vehicle[1]["Engine"], vehicle[1]["Health"], LsCustoms,
												vehicle[1]["Windows"], vehicle[1]["Tyres"], vehicle[1]["Brakes"], false)
											TriggerClientEvent("Notify", source, "Próximo pagamento",
												CompleteTimers(vehicle[1]["Tax"] - os.time()), "azul", 5000)
											Entity(Networked)["state"]:set("Fuel", vehicle[1]["Fuel"], true)
											TriggerEvent("engine:InsertBrakes", Network, vehicle[1]["Brakes"])
											Spawn[Plate] = { Passport, Name, Network }
										...
										if vRP.Request(source, "Garagem", "Retirar o veículo por <b>$" .. Dotted(VehiclePrice * 0.05) .. "</b> dólares?") then
											if vRP.PaymentFull(Passport, VehiclePrice * 0.05) then
												TriggerClientEvent("dynamic:Close", source)
												local Exist, Network = Hensa.ServerVehicle(Name, Coords[1], Coords[2], Coords[3], Coords[4],
												...
												TriggerClientEvent("Notify", source, "Aviso",
													"<b>" .. ItemName(DefaultMoneyOne) .. "</b> insuficientes.", "vermelho", 5000)
											end
										end
									end
								end
...
RegisterCommand("car", function(source, Message)
	...
	if Passport then
		if vRP.HasGroup(Passport, CarPermission) and Message[1] then
			...
			local Exist, Network, Vehicle = Hensa.ServerVehicle(VehicleName, Coords["x"], Coords["y"], Coords["z"], Heading,
				Plate, 2000, nil, 1000)
			if not Exist then
				return
			end
...
AddEventHandler("garages:Lock", function(Network, Plate, Model)
	...
	if Passport and GlobalState["Plates"][Plate] == Passport then
		if Player(source)["state"]["BlockLocked"] then
			...
			TriggerClientEvent("Notify", source, "Atenção",
				"Para a segurança de todos, o veículo não pode ser trancado no momento.", "amarelo", 5000)
		else
...
AddEventHandler("garages:DeleteVehicle", function(Network, Plate)
	if Network ~= nil and Plate ~= nil then
		...
		local NitroState = GlobalState["Nitro"]
		if NitroState and NitroState[Plate] then
			local Nitro = NitroState
			Nitro[Plate] = nil
			GlobalState:set("Nitro", Nitro, true)
		end
	...
	if Network ~= nil and Plate ~= nil then
		...
		if string.sub(Plate, 1, 4) == "DISM" then
			...
			if source then
				...
				TriggerClientEvent("Notify", source, "Jimmy Jango",
					"O seu serviço foi finalizado com sucesso, e você pode assinar um novo contrato quando quiser.", "verde", 10000)
			end
		end
