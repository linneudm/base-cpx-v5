...
exports("Drops", function(Passport, source, Item, Amount, Force, Coords)
	...
	if not Provisory.desc then
		if NameItem == "vehiclekey" and Split[3] then
			...
			if Consult and VehicleExist(Consult.Vehicle) then
				Provisory.desc = ("Proprietário: <common>%s</common><br>Modelo: <common>%s</common><br>Placa: <common>%s</common>")
						:format(vRP.FullName(Consult.Passport), VehicleName(Consult.Vehicle), Split[3])
			end
		...
		elseif ItemNamed(NameItem) and Split[2] and vRP.Identity(Split[2]) then
			if NameItem == "identity" then
				Provisory.desc = ("Passaporte: <rare>%s</rare><br>Nome: <rare>%s</rare><br>Telefone: <rare>%s</rare>"):format(
					Dotted(Split[2]), vRP.FullName(Split[2]), vRP.Phone(Split[2]))
			else
	...
	Provisory.data = {
		Name = ItemName(Provisory.key),
		Weight = ItemWeight(Provisory.key),
		Index = ItemIndex(Provisory.key),
		Economy = ItemEconomy(Provisory.key),
		Rarity = ItemRarity(Provisory.key),
		Description = Provisory.desc or ItemDescription(Provisory.key)
	}
...
function Creative.Pickup(Number, Route, Target, Amount)
	...
	if vRP.CheckWeight(Passport, Info.key, Amount) then
		...
		local cannotReceive = not Info.amount
				or Info.amount < Amount
				or (Inv[Target] and Inv[Target].item ~= Info.key)
				or vRP.MaxItens(Passport, Info.key, Amount)

		if cannotReceive then
			TriggerClientEvent("inventory:Notify", source, "Aviso", "Mochila Sobrecarregada.", "amarelo")
		else
			-- Apos as validacoes acima, o GiveItem deve ocorrer.
			-- Nao dependemos do retorno para evitar duplicacao de drop por retorno nil em implementacoes antigas.
			vRP.GiveItem(Passport, Info.key, Amount, false, Target)
