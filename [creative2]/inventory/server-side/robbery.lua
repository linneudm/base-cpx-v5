	...
	if Configuration.Need then
		...
		if not RequiredItem then
			TriggerClientEvent("Notify", source, "Atenção",
				"Precisa de <b>" .. Configuration.Need.Amount .. "x " .. ItemName(Configuration.Need.Item) .. "</b>.", "amarelo",
				5000)
			return false
		end
	end
		...
		if Active[Passport] then
			...
			if (not Configuration.Need) or (RequiredItem and (not Configuration.Need.Consume or vRP.TakeItem(Passport, RequiredItem.Item, Configuration.Need.Amount))) then
				...
				if Configuration.Explosion then
					...
					exports["inventory"]:Drops(Passport, source, Configuration.Payment.Money.Item, Valuation, false,
						Robbery[Number].Coords)
				else
