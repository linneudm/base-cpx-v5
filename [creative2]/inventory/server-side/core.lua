	...
	if Passport and not Active[Passport] and ClosestPed and not exports["hud"]:Wanted(Passport) then
		...
		if not vRP.MaxItens(OtherPassport, Item, Amount) then
			if vRP.CheckWeight(OtherPassport, Item, Amount) then
				...
				vRPC.CreateObjects(source, "mp_safehouselost@", "package_dropoff", "prop_paper_bag_small", 16, 28422, 0.0, -0.05,
					0.05, 180.0, 0.0, 0.0)
	...
	if Passport and not Active[Passport] then
		...
		elseif Work == "Transporter" then
			if not vRPC.LastVehicle(source, "stockade") then
				TriggerClientEvent("Notify", source, "Atenção", "Precisa utilizar o veículo do <b>Transportador</b>.", "amarelo",
					5000)
				Active[Passport] = nil
...
function Creative.Use(Slot, Amount)
	...
	if Passport and not Active[Passport] then
		...
		local WaterCondition = WaterItem and
				((WaterItem == "Out" and WaterEntity) or (WaterItem == "In" and not WaterEntity))
			...
			if Weapon then
				...
				if Component then
					...
					if not Check then
						...
						TriggerClientEvent("inventory:Notify", source, "Atenção", "O armamento já possui um componente equipado.",
							"vermelho")
					end
				else
					TriggerClientEvent("inventory:Notify", source, "Atenção", "O armamento não possui suporte ao componente.",
						"vermelho")
				end
			end
...
AddEventHandler("inventory:Loot", function(Number, Box)
	...
	if Passport and Loots[Box] then
		...
		if Loots[Box]["Item"] then
			...
			if not Consult then
				TriggerClientEvent("Notify", source, "Atenção", "Precisa de <b>1x " .. ItemName(Loots[Box]["Item"]) .. "</b>.",
					"amarelo", 5000)
				return false
			end
		end
		...
		if Loots[Box]["Players"][Number][Passport] then
			if os.time() <= Loots[Box]["Players"][Number][Passport] then
				TriggerClientEvent("Notify", source, "Atenção",
					"Aguarde " .. CompleteTimers(Loots[Box]["Players"][Number][Passport] - os.time()) .. ".", "amarelo", 5000)
				return false
			end
		end
			...
			if Active[Passport] then
				...
				if not Loots[Box]["Item"] or (Loots[Box]["Item"] and Consult and vRP.RemoveCharges(Passport, Loots[Box]["Item"])) then
					...
					if not vRP.MaxItens(Passport, Result["Item"], Result["Valuation"]) and vRP.CheckWeight(Passport, Result["Item"], Result["Valuation"]) then
						...
						TriggerClientEvent("Notify", source, "Mochila Sobrecarregada", "Sua recompensa caiu no chão.", "amarelo",
							5000)
...
AddEventHandler("inventory:ChangePlate", function(Entitys)
	...
	if Passport and not Active[Passport] and not Plates[Plate] then
		if not vRP.ConsultItem(Passport, "plate", 1) then
			TriggerClientEvent("Notify", source, "Atenção", "Precisa de <b>1x " .. ItemName("plate") .. "</b>.", "amarelo",
				5000)
...
AddEventHandler("inventory:Products", function(Service)
	...
	if Passport and not Active[Passport] and Products[Service] then
		...
		if Products[Service]["Item"] and not vRP.ConsultItem(Passport, Products[Service]["Item"]) then
			TriggerClientEvent("Notify", source, "Atenção", "Precisa de <b>1x " .. ItemName(Products[Service]["Item"]) ..
				"</b>.", "amarelo", 5000)
		...
		if Products[Service]["Animation"] then
			vRPC.playAnim(source, false, { Products[Service]["Animation"]["Dict"], Products[Service]["Animation"]["Anim"] },
				true)
		end
...
exports("Buffs", function(Mode, Passport)
	return Buffs[Mode] and Buffs[Mode][Passport] and Buffs[Mode][Passport] > os.time() and
			(Mode ~= "Luck" or (Mode == "Luck" and math.random(100) >= 50)) and true or false
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- CLEANWEAPONS
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("SaveServer", function(Silenced)
	...
	for Passport, _ in pairs(List) do
		...
		if Users["Blueprints"] and Users["Blueprints"][Passport] then
			vRP.Query("playerdata/SetData",
				{ Passport = Passport, Name = "Blueprints", Information = json.encode(Users["Blueprints"][Passport]) })
		end
	end
