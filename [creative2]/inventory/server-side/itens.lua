...
Use = {
	...
	["radiomhz"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		if Keyboard then
			...
			if Frequency and string.len(Frequency) >= 1 and string.sub(Frequency, 1, 1) ~= "0" then
				if not exports.radio:RadioExist(Frequency) and vRP.TakeItem(Passport, Full, 1, false, Slot) then
					TriggerClientEvent("Notify", source, "Sucesso", "Frequência adicionada.", "verde", 5000)
					exports.radio:RadioAdd(Frequency, Keyboard[2])
				end
			else
				TriggerClientEvent("Notify", source, "Negado", "Precisa ter no mínimo 1 número e não pode começar com zero.",
					"vermelho", 5000)
			end
		end
	end,
	...
	["identity"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if Split[2] then
			local TargetPassport = parseInt(Split[2])

			if TargetPassport and TargetPassport > 0 then
				local IdentityData = Creative.Identity(TargetPassport)

				if IdentityData and IdentityData.Passport and IdentityData.Passport > 0 then
					local DataArray = {
						IdentityData.Avatar or "images/user.svg",
						IdentityData.Passport or 0,
						IdentityData.Name or "Desconhecido",
						IdentityData.Sex or "M",
						IdentityData.Gemstone or 0,
						IdentityData.Job or "Desempregado"
					}
					TriggerClientEvent("inventory:Identity", source, DataArray)
				else
					TriggerClientEvent("inventory:Notify", source, "Erro", "Dados de identidade não encontrados.", "vermelho")
				end
			else
				TriggerClientEvent("inventory:Notify", source, "Erro", "Passaporte de destino inválido.", "vermelho")
			end
		else
			TriggerClientEvent("inventory:Notify", source, "Erro", "Identidade inválida.", "vermelho")
		end
	end,
	...
	["vape"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		vRPC.CreateObjects(source, "anim@heists@humane_labs@finale@keycards", "ped_a_enter_loop", "ba_prop_battle_vape_01",
			49, 18905, 0.08, -0.00, 0.03, -150.0, 90.0, -10.0)
				...
				CreateThread(function()
					...
					if Active[Passport] then
						...
						if vRP.TakeItem(Passport, Full, 1, true, Slot) then
							...
							TriggerClientEvent("Notify", source, "Informações",
								"<b>Químicos:</b> " .. chemStr .. "<br><b>Álcool:</b> " .. alcoholStr .. "<br><b>Drogas:</b> " .. weedStr,
								"amarelo", 8000)
						end
					end
				end)
				...
				if (not Travel[Passport] or #(vRP.GetEntityCoords(source) - Travel[Passport]) >= 100) then
					...
					if vDEVICE.Device(source, 30) then
						...
						if Boosting[Plate].Amount >= 10 then
							...
							TriggerClientEvent("Notify", source, "Boosting [ " .. Boosting[Plate].Amount .. " / 10 ]",
								"Progresso atualizado com sucesso.", "verde", 5000)
						end
					else
						...
						TriggerClientEvent("Notify", source, "Boosting [ " .. Boosting[Plate].Amount .. " / 10 ]",
							"Progresso atualizado com sucesso.", "amarelo", 5000)
					end
				end
			else
				TriggerClientEvent("inventory:Close", source)
	...
	["cola"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		TriggerClientEvent("inventory:Close", source)
		TriggerClientEvent("Progress", source, "Bebendo", 5000)
		vRPC.CreateObjects(source, "mp_player_intdrink", "loop_bottle", "prop_ecola_can", 49, 60309, 0.01, 0.01, 0.05, 0.0,
			0.0, 90.0)
	...
	["sandwich"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		TriggerClientEvent("inventory:Close", source)
		TriggerClientEvent("Progress", source, "Comendo", 5000)
		vRPC.CreateObjects(source, "mp_player_inteat@burger", "mp_player_int_eat_burger", "prop_sandwich_01", 49, 18905, 0.13,
			0.05, 0.02, -50.0, 16.0, 60.0)
	...
	["tyres"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if not vRP.InsideVehicle(source) then
			...
			if Vehicle then
				...
				vRPC.CreateObjects(source, "anim@heists@box_carry@", "idle", "imp_prop_impexp_tyre_01a", 49, 28422, -0.02, -0.1,
					0.2, 10.0, 0.0, 0.0)
	...
	if v.Item then
		Use["vehicle_" .. Model] = function(source, Passport, Amount, Slot, Full)
			if vRP.SelectVehicle(Passport, Model) then
				TriggerClientEvent("inventory:Notify", source, "Aviso", "Já possui um <b>" .. VehicleName(Model) .. "</b>.",
					"amarelo")
				return false
			end
			...
			if vRP.TakeItem(Passport, Full, 1, false, Slot) then
				...
				if type(v.Item) == "number" then
					vRP.Query("vehicles/rentalVehicles",
						{ Passport = Passport, Vehicle = Model, Plate = Plate, Days = v.Item, Weight = Weight, Work = 0 })
				elseif v.Item == "Permanent" then
					vRP.Query("vehicles/addVehicles",
						{ Passport = Passport, Vehicle = Model, Plate = Plate, Weight = Weight, Work = 0 })
				end

				TriggerClientEvent("inventory:Notify", source, "Sucesso", "Veículo <b>" .. VehicleName(Model) ..
					"</b> adicionado.", "verde")
				TriggerClientEvent("inventory:Update", source)
			end
		end
	...
	Use[NameItem] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		if vRPC.SprayExist(source, 500) then
			TriggerClientEvent("Notify", source, "Atenção",
				"No momento você não pode prosseguir porque outro grupo está dominando a localidade.", "amarelo", 5000)
			return false
		end
		...
		if Application and Coords then
			...
			vRPC.CreateObjects(source, "switch@franklin@lamar_tagging_wall", "lamar_tagging_wall_loop_lamar",
				"prop_cs_spray_can", 1, 28422)
			...
			SetTimeout(5000, function()
				if Active[Passport] then
					...
					vRPC.CreateObjects(source, "switch@franklin@lamar_tagging_wall", "lamar_tagging_exit_loop_lamar",
						"prop_cs_spray_can", 1, 28422)
