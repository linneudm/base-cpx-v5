...
-- INVENTORY:IDENTITY
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("inventory:Identity")
AddEventHandler("inventory:Identity", function(Data)
	if Data and #Data >= 6 then
		...
		SetCursorLocation(0.5, 0.5)
		TriggerEvent("hud:Active", false)
		SendNUIMessage({ Action = "Identity", Payload = Data })
	else
		TriggerEvent("inventory:Notify", "Erro", "Dados de identidade inválidos.", "vermelho")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVENTORY:CLOSE
...
RegisterNUICallback("Update", function(Data, Callback)
	local Updated = vRPS.UpdateInventory(Data.slot, Data.target, Data.amount)
...
RegisterNUICallback("Mount", function(Data, Callback)
	...
	if Primary then
		local Secondary = {}
		local Ped = PlayerPedId()
		local Route = tonumber(LocalPlayer.state.Route) or 0
		...
		Callback({
			...
			Secondary = Secondary,
			PrimaryMaxWeight = MaxWeight,
			SecondarySlots = math.max(
				CountTable(Secondary), 25)
		})
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
