...
AddEventHandler("routes:Open", function(Name)
	if not Progress or (Progress and Progress == Name) then
		...
		TriggerEvent("Notify", "Atenção", "Volte no emprego de <b>" .. Progress .. "</b> e finalize o mesmo.", "amarelo",
			5000)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		if Progress then
			...
			if not IsPedInAnyVehicle(Ped) then
				...
				if Distance <= 10.0 then
					TimeDistance = 1
					SetDrawOrigin(Initial[Progress]["Coords"][Selectedz]["x"], Initial[Progress]["Coords"][Selectedz]["y"],
						Initial[Progress]["Coords"][Selectedz]["z"])
...
RegisterCommand("FinishRoute", function()
	print("FinishRoute")
	if Progress then
		vSERVER.Finish()
		Actived = false
...
RegisterKeyMapping("FinishRoute", "Finalizar a rota.", "keyboard", "F7")
