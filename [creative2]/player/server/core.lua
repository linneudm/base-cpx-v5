...
AddEventHandler("player:Recycle", function()
	...
	if Passport and not Recycled[Passport] and not exports["hud"]:Repose(Passport, source) and not exports["hud"]:Wanted(Passport, source) and vRP.Request(source, "Recicladora", "Está apenas com itens que deseja reciclar em sua mochila?") then
		local Notify = false
...
AddEventHandler("player:Death", function(OtherSource)
	...
	if Passport and OtherPassport and Passport ~= OtherPassport and vRP.DoesEntityExist(source) and vRP.DoesEntityExist(OtherSource) then
		exports["discord"]:Embed("Deaths",
			"**Passaporte do Assassino:** " ..
			OtherPassport ..
			"\n**Localização do Assassino:** " ..
			vRP.GetEntityCoords(OtherSource) ..
