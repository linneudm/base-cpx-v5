				...
				if atmTimers[user_id] then
					if atmTimers[user_id][1] > 0 and atmTimers[user_id][2] >= 5000 then
						TriggerClientEvent("Notify", source, "azul",
							"Aguarde " .. vRP.getTimers(parseInt(atmTimers[user_id][1])) .. "</b>.", 5000)
						return vRP.getBank(user_id)
					end
		...
		if parseInt(amount) > 0 then
			if bugatm == 0 then
				...
				if amount == deposito then
					TriggerClientEvent("Notify", source, "vermelho", "Você acabou de depositar mesmo valor, tente mais tarde.",
						10000)
				else
					deposito = amount
		...
		if nuser_id ~= user_id then
			if tonumber(amount) >= 0 and tonumber(amount) <= 3000 then
				if banco > tonumber(amount) then
					if transferido == 0 then
						...
						SetTimeout(3000, function()
							...
							TriggerClientEvent("Notify", nplayer, "dinheiro",
								"<b>" .. identity.name ..
								" " .. identity.name2 .. "</b> depositou <b>$" .. amount .. " dólares</b> na sua conta.", 8000)
							TriggerClientEvent("Notify", source, "dinheiro",
								"Você transferiu <b>$" ..
								amount .. " dólares</b> para conta de <b>" .. identity2.name .. " " .. identity2.name2 .. "</b>.", 8000)
						end)
					else
						TriggerClientEvent("atmCloseEvent", source)
				...
				TriggerClientEvent("Notify", source, "negado",
					"Caixas eletronicos só aceitam até 3.000,00 dólares de transferencia.", 5000)
			end
		else
			TriggerClientEvent("atmCloseEvent", source)
