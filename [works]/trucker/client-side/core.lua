...
AddEventHandler("trucker:initVehicles", function()
	if not Package and not vSERVER.checkExist() then
		...
		TriggerEvent("Notify", "amarelo",
			"Dirija-se até seu caminhão, buzine o mesmo<br>para receber a carga responsável pelo transporte.", 5000)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- TRUCKER:INITDIESEL
			...
			if IsEntityAVehicle(Vehicle) then
				...
				if Truckers[vehModel] then
					...
					if distance <= 200 then
						timeDistance = 1
						DrawMarker(1, packService[Service]["coords"][Position][1], packService[Service]["coords"][Position][2],
							packService[Service]["coords"][Position][3] - 3, 0, 0, 0, 0, 0, 0, 12.0, 12.0, 8.0, 255, 255, 255, 25, 0, 0,
							0, 0)
						DrawMarker(21, packService[Service]["coords"][Position][1], packService[Service]["coords"][Position][2],
							packService[Service]["coords"][Position][3] + 1, 0, 0, 0, 0, 180.0, 130.0, 3.0, 3.0, 2.0, 42, 137, 255, 100,
							0, 0, 0, 1)
						if distance <= 10 then
							if Position >= #packService[Service]["coords"] then
								...
								if Position == 1 then
									if IsControlJustPressed(1, 38) then
										...
										local Trailer = vGARAGE.serverVehicle(packService[Service]["trailer"], vehCoords["x"], vehCoords
											["y"], vehCoords["z"], Heading, nil, 0, nil, 1000)
...
function makeBlipMarked()
	...
	Blip = AddBlipForCoord(packService[Service]["coords"][Position][1], packService[Service]["coords"][Position][2],
		packService[Service]["coords"][Position][3])
