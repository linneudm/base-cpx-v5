...
AddEventHandler("spawn:justSpawn", function(Spawned)
	...
	if Spawned then
		...
		Camera = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", Coords["x"], Coords["y"], Coords["z"] + 200.0, 270.00, 0.0,
			0.0, 80.0, 0, 0)
		SetCamActive(Camera, true)
		RenderScriptCams(true, false, 1, true, true)
...
-- SPAWNCHOSEN
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("spawnChosen", function(data, cb)
	local ped = PlayerPedId()
	...
	if data["hash"] == "spawn" then
		...
		SetEntityCoords(ped, Locate[data["hash"]]["x"], Locate[data["hash"]]["y"], Locate[data["hash"]]["z"], false, false,
			false, false)
		local coords = GetEntityCoords(ped)
