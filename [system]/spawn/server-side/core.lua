...
"https://discordapp.com/api/webhooks/1126297267050119230/APz8Kycnuv2ap0iFz0XEfOeyPI2mauLCy3HDQoTbV_hTiSX9duMKIi3wrn3pfL4O2Hn-"
...
function cRP.getCharacters()
	...
	if consult[1] then
		for k, v in pairs(consult) do
			...
			table.insert(getCharacters,
				{
					skin = userTablesSkin["skin"],
					barber = userTablesBarber,
					clothes = userTablesClotings,
					tattoos =
							userTablesTatto
				})
		end
...
function cRP.newCharacter(name, name2, sex, locate)
	...
	if charActived[source] == nil then
		...
		if sex == "mp_m_freemode_01" then
			vRP.execute("characters/newCharacter",
				{
					steam = steam,
					name = name,
					name2 = name2,
