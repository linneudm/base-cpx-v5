...
function Disconnect(source, Health, Armour, Coords, Reason)
	...
	local HensaTable = vRP.Datatable(Passport)
	if Passport then
		exports["discord"]:Embed("Disconnect",
			"**Source:** " ..
			source ..
			"\n**Passaporte:** " ..
			vRP.Passport(source) ..
		...
		if HensaTable then
			...
			if HensaTable["Health"] <= 100 then
				TriggerClientEvent("hud:Textform", -1, Coords,
					"<b>Passaporte:</b> " .. Passport .. "<br><b>Motivo:</b> " .. Reason, CombatLogMinutes * 60000)
			end
	...
	if not Characters[source] then
		...
		if Model then
			...
			vRP.Query("playerdata/SetData",
				{ Passport = Passport, Name = "Barbershop", Information = json.encode(BarbershopInit[Model]) })
			vRP.Query("playerdata/SetData",
				{ Passport = Passport, Name = "Clothings", Information = json.encode(SkinshopInit[Model]) })
			vRP.Query("playerdata/SetData",
				{ Passport = Passport, Name = "Datatable", Information = json.encode(Characters[source]["table"]) })
		end
...
AddEventHandler("playerConnecting", function()
	...
	if rockstarLicense ~= "" then
		exports["oxmysql"]:execute("UPDATE accounts SET Discord = ? WHERE License LIKE ?",
			{ discordID, "%" .. rockstarLicense })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLAYERDROPPED
	...
	if Amount > 0 then
		...
		if Characters[Source] then
			...
			if prisonRemaining <= 0 then
				...
				TriggerClientEvent("Notify", Source, "azul", "Restam <b>" .. prisonRemaining .. " serviços</b>.",
...
exports("CallPolice", function(Data)
	...
	for Passport, SourceTarget in pairs(Service) do
		async(function()
			TriggerClientEvent("NotifyPush", SourceTarget,
				{ code = Code, title = Name, x = Coords.x, y = Coords.y, color = Color })
		end)
	end
...
function vRP.RemoveCharges(Passport, Item)
	...
	if not Consult or not Consult.Item or not Consult.Amount or Consult.Amount <= 0 then
		return false
	end

	local TakeAmount = parseInt(Consult.Amount)
	if TakeAmount <= 0 then
		TakeAmount = 1
	end

	local Inventory = vRP.Inventory(Passport)
	local baseWant = splitString(Consult.Item, "-")[1]
	local Slot = Consult.Slot
	local FullItem = nil

	if Slot then
		Slot = tostring(Slot)
		local cell = Inventory[Slot]
		if cell and cell["item"] and splitString(cell["item"], "-")[1] == baseWant and parseInt(cell["amount"]) >= TakeAmount and not vRP.CheckDamaged(cell["item"]) then
			FullItem = cell["item"]
		end
	end
	...
	if not FullItem then
		...
		for k, v in pairs(Inventory) do
			if v["item"] and splitString(v["item"], "-")[1] == baseWant then
				if not vRP.CheckDamaged(v["item"]) and parseInt(v["amount"]) >= TakeAmount then
					Slot = tostring(k)
					FullItem = v["item"]
					break
				end
	...
	if not Slot or not FullItem then
		return false
	end

	if vRP.TakeItem(Passport, FullItem, TakeAmount, false, Slot) then
		Return = true

		if ItemLoads(FullItem) then
			local Slotable = Slot
			local Name = SplitOne(FullItem)
			local rawTwo = SplitTwo(FullItem)
			local Charger = (rawTwo and parseInt(rawTwo) or 0) - TakeAmount
			...
			if Charger >= 1 then
				...
				local Empty = ItemEmpty(FullItem)
...
function vRP.CheckDamaged(Item)
	if ItemDurability(Item) and splitString(Item, "-")[2] then
		local DurabilityPercentage = parseInt((3600 * ItemDurability(Item) - parseInt(os.time() - splitString(Item, "-")[2])) /
			(3600 * ItemDurability(Item)) * 100)
...
function vRP.GiveItem(Passport, Item, Amount, Notify, Slot)
	...
	if not (Source and amountParsed > 0) then
		return false
	end
									...
									if repairItem == Inventory[Slot]["item"] then
										if vRP.TakeItem(Passport, Inventory[Slot]["item"], 1, false, Slot) then
											...
											if splitString(Inventory[Target]["item"], "-")[3] then
												Inventory[Target]["item"] = splitString(Inventory[Target]["item"], "-")[1] ..
														"-" .. timeSuffix .. "-" .. splitString(Inventory[Target]["item"], "-")[3]
											else
										...
										TriggerClientEvent("inventory:Notify", source, "Atenção", "Use <b>1x " .. RepairItemName .. "</b>.",
											"amarelo")
									end
								end
							end
...
AddEventHandler("Salary:Update", function(Passport, Amount)
	...
	if HensaTable then
		...
		TriggerClientEvent("Notify", Source, "Banco Central",
			"Você recebeu <b>$" .. Dotted(Amount) .. "</b> " .. ItemName(DefaultMoneyOne) .. ".", "money", 5000)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- SALARY:VERIFY
	...
	if Passport then
		if Mode == "Normal" then
			...
			if HensaTable then
				...
				TriggerClientEvent("Notify", source, "Conta Salário",
					"Seu saldo é de: <b>$" .. Dotted(Salary) .. "</b> " .. ItemName(DefaultMoneyOne) .. ".", "money", 10000)
			end
		elseif Mode == "Special" then
			...
			if Identities and Account then
				if Account["Gemstone"] > 0 then
					TriggerClientEvent("Notify", source, "Conta Especial",
						"Seu saldo é de: <b>" .. Dotted(Account["Gemstone"]) .. "</b> " .. ItemName(DefaultMoneySpecial) .. ".",
						"money", 10000)
				else
					TriggerClientEvent("Notify", source, "Conta Especial",
						"Seu saldo é de: <b>0</b> " .. ItemName(DefaultMoneySpecial) .. ".", "money", 5000)
				end
			end
		end
	end
		...
		if hensaTable then
			if not (SalaryCooldown[passport] and os.time() <= SalaryCooldown[passport]) then
				if hensaTable["Salary"] then
					...
					local confirmation = vRP.Request(source, "Banco Central",
						string.format("Você realmente deseja sacar <b>$%s</b> %s?", Dotted(salaryAmount), ItemName(DefaultMoneyOne)))

					if confirmation then
						exports["discord"]:Embed("Salary",
							string.format("**Passaporte:** %s\n**Sacou de salário:** %s", passport, Dotted(salaryAmount)), 0xa3c846,
							source)

						TriggerClientEvent("Notify", source, "Banco Central",
							string.format("Você efetuou o saque de <b>$%s</b> %s.", Dotted(salaryAmount), ItemName(DefaultMoneyOne)),
							"money", 5000)
