...
function vRP.addBank(user_id,amount,mode)
	if parseInt(amount) > 0 then
		...
		if parseInt(amount) > 500000 then
			TriggerEvent("discordLogs","addValueBank","**Passaporte:** "..parseFormat(user_id).."\n**Depositou:** ".." $"..parseFormat(amount).."\n**Horário:** "..os.date("%H:%M:%S"),3042892)
			vRP.execute("log/addValueBank",{ user_id = user_id, value = amount })
		end
