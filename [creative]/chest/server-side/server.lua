		...
		if mychestname ~= nil then
			...
			if data then
				for k, v in pairs(sdata) do
					table.insert(mychestopen,
						{
							desc = vRP.itemDescList(k),
							unity = vRP.itemUnityList(k),
							tipo = vRP.itemTipoList(k),
							economy = vRP
									.itemEconomyList(k),
			...
			if consult[1].name == mychestname then
				return myinventory, mychestopen, vRP.computeInvWeight(user_id), vRP.getBackpack(user_id),
						vRP.computeChestWeight(sdata), consult[1].weight,
						{ identity.name .. " " .. identity.name2, parseInt(user_id), identity.phone, identity.registration, vRP
								.getBank(user_id) }
			end
		end
	end
