...
AddEventHandler("chest:openSystem", function(shopId)
	if LocalPlayer["state"]["Route"] < 900000 then
		local perm = vSERVER.Permissions(chestCoords[shopId][1])
		if perm and MumbleIsConnected() then
			SetNuiFocus(true, true)
			chestOpen = chestCoords[shopId][1]
			--SendNUIMessage({ action = "showMenu" })
			TriggerEvent("inventory:openBackpack", { chest = "chest" })
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterNUICallback("takeItem", function(data, cb)
	if MumbleIsConnected() then
		vSERVER.takeItem(data["slot"], data["amount"], data["target"], chestOpen)
	end
	cb("ok")
end)
-----------------------------------------------------------------------------------------------------------------------------------------
