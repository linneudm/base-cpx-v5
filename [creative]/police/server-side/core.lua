...
function cRP.setWarrant(passport, reason)
	...
	vRP.execute("warrants/setWarrant",
		{
			user_id = passport,
			identity = identity["name"] .. identity["name2"],
			status = "Procurado",
			nidentity = nidentity
					["name"] .. nidentity["name2"],
			timeStamp = os.date('%d/%m/%Y', os.time()),
			reason = reason
		})
	...
	if user_id then
		if actived[user_id] == nil then
			...
			if identity then
				...
				vRP.execute("prison/insertPrison",
					{
...
function cRP.searchUser(nuser_id)
	...
	if user_id then
		...
		if identity then
			...
			return { true, identity["name"] .. " " .. identity["name2"], identity["phone"], fines, records, nil, port, identity
					["locate"] }
		end
	end
