...
const benefactor = (mode) => {
	...
	$("#content").html(`
		<div id="benefactorBar">
			...
			<li id="benefactor" data-id="Aluguel" ${mode == "Aluguel" ? "class=active":""}>PREMIUM</li>
		</div>
	...
	$.post("http://tablet/request"+ mode,JSON.stringify({}),(data) => {
		...
		if (mode !== "Possuidos"){
			$("#pageVehicles").html(`
				...
				<img src="http://181.214.221.76/cars_images/${item.k}.png" style="margin-left: 3.5vw; margin-top: 1vw; width: 7vw; height: 4vw;">
					<left>
						${item["name"]}<br>
					...
					<right>
						${mode == "Aluguel"?"<div id=\"benefactorRental\"data-name="+item["k"]+">PREMUIM</div>":"<div id=\"benefactorBuy\" data-name="+item["k"]+">PREMIUM</div>"}
			...
			$("#pageVehicles").html(`
				${nameList.map((item) => (`<span>
				<img src="nui://tablet/web-side/cars_images/${item.k}.png" style="margin-left: 3.5vw; margin-top: 1vw; width: 7vw; height: 4vw;">
					<left>
						${item["name"]}<br>
						<b>Valor:</b> ${mode == "Aluguel" ? format(item["price"])+" Gemas":"$"+format(item["price"])}<br>
						<b>Porta-Malas:</b> ${format(item["chest"])}Kg
