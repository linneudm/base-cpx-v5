		...
		if IsPedInAnyVehicle(ped) then
			...
			if NetworkGetEntityIsNetworked(vehicle) then
				...
				if IsVehicleEngineOn(vehicle) and GetPedInVehicleSeat(vehicle, -1) == ped and GetVehicleFuelLevel(vehicle) >= 5 and speed > 5 then
					...
					TriggerServerEvent("engine:tryFuel", VehToNet(vehicle),
						GetVehicleFuelLevel(vehicle) -
						(vehFuel[floor(GetVehicleCurrentRpm(vehicle))] or 1.0) * (vehClass[GetVehicleClass(vehicle)] or 1.0) / 10)
				end
			end
		end
							...
							if distance <= 3.5 then
								if not isFuel then
									...
									if vehFuel >= 0.0 then
										...
										text3D(coordsVeh.x, coordsVeh.y, coordsVeh.z + 0.85,
											"TANQUE: ~y~" .. parseInt(floor(vehFuel)) .. "%  ~w~PREÇO: ~y~$" .. parseInt(isPrice))
...
AddEventHandler("engine:vehTuning", function()
	...
	if vehicle and NetworkGetEntityIsNetworked(vehicle) then
		...
		TriggerEvent("Notify", "default",
			...
			blindagem ..
			"<br><b>Lataria:</b> " ..
			parseInt(body / 10) .. "%<br><b>Motor:</b> " .. parseInt(engine / 10) ..
			"%<br><b>Gasolina:</b> " .. parseInt(fuel) .. "%", 10000)
	end
end)
