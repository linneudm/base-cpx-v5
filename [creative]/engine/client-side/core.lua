							...
							if DoesEntityExist(vehicle) and not vehiclesBlacklist[vehModel]  then
								...
								if not pumpGunInHand and distanceToPump < 2.0 and vehFuel < 100.0 then
									...
									if IsControlJustPressed(1,38) and GetGameTimer() >= gameTimer then
										...
										--SetNetworkIdExistsOnAllMachines(pumpNetId, true)
										--NetworkSetNetworkIdDynamic(pumpNetId, true)
										--SetNetworkIdCanMigrate(pumpNetId, false)
										...
										local RopePumpOffset = GetOffsetFromEntityInWorldCoords(currentPumpEntity, 0.0, -0.02, -0.175) --GetEntityCoords(currentPumpEntity)
								...
								if pumpGunInHand and distanceToVeh <= 3.5 then
									if not isFuel then
										...
										if not showNui then
											...
											--if v[6] then
											--	Title = "Ipiranga"
											--	Legends = "Gasolina adtivada"
											--	Background = "url(dtclean.png) rgba(0,41,148,.75)"
											--end
								...
								if isFuel and distanceToVeh > 3.5 then
									...
									SetTimeout(3000,function()
										...
										if distanceToPump > 7.0+(lastVehicleIsPlane and 20.0 or 0.0) and pumpGunInCar then
											explodePump = true
											AddExplosion(lasTcoords.x,lasTcoords.y,lasTcoords.z, 2, 1.0, 1.0, true, true, false)
										end
										lastVehicleInFuel = nil
										lastVehicleIsPlane = nil
										pumpGunInHand = false
