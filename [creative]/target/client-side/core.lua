	...
	for k, v in pairs(Exclusives) do
		if v[4] == "Desmanche" then
			TriggerEvent("NotifyPush",
				{ code = 20, title = "Localização do Desmanche", x = v[1], y = v[2], z = v[3], blipColor = 60 })
			break
		end
	end
end)
...
CreateThread(function()
	while true do
		if LocalPlayer["state"]["Target"] then
...
CreateThread(function()
	...
	AddTargetModel(
		{ 1211559620, 1363150739, -1186769817, 261193082, -756152956, -1383056703, 720581693, 1287257122, 917457845, -838860344 },
		{
			options = {
				{
					event = "inventory:verifyObjects",
							...
							if not Combustivel then
								if GetSelectedPedWeapon(ped) == 883325847 then
									...
									if GlobalState["vehPlates"][vehPlate] then
											...
											for k, Tyre in pairs(tyreList) do
												...
												if Wheel ~= -1 then
													...
													if Distance <= 1.0 then
														Selected[5] = Tyre
														table.insert(Menu,
															{ event = "inventory:removeTyres", label = "Retirar Pneu", tunnel = "client" })
													end
												end
											end

											table.insert(Menu,
												{ event = "trunkchest:openTrunk", label = "Abrir Porta-Malas", tunnel = "server" })
										end
										...
										if GetVehicleDoorLockStatus(entity) == 1 then
											table.insert(Menu,
												{ event = "player:enterTrunk", label = "Entrar no Porta-Malas", tunnel = "client" })
										end
										...
										if Rolling > 75.0 or Rolling < -75.0 then
											...
											if GetEntityBoneIndexByName(entity, "boot") ~= -1 then
												...
												if Distance <= 1.25 then
													table.insert(Menu,
														{ event = "inventory:stealTrunk", label = "Arrombar Porta-Malas", tunnel = "client" })
												end
											end
										end
									end
						...
						if #(coords - entCoords) <= 1.0 then
							...
							elseif LocalPlayer["state"]["Paramedic"] then
								if GetEntityHealth(entity) <= 100 then
									...
									table.insert(Menu,
										{ event = "paramedic:presetBurn", label = "Roupa de Queimadura", tunnel = "paramedic" })
