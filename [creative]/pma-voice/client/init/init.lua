...
AddEventHandler('onClientResourceStart', function(resource)
	...
	-- Reinitialize channels if they're valid numeric values.
	local savedRadioChannel = tonumber(LocalPlayer.state.radioChannel)
	if savedRadioChannel and savedRadioChannel > 0 then
		setRadioChannel(savedRadioChannel)
	end

	local savedCallChannel = tonumber(LocalPlayer.state.callChannel)
	if savedCallChannel and savedCallChannel > 0 then
		setCallChannel(savedCallChannel)
	end
