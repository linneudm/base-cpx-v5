	...
	if user_id then
		...
		if vehicle[1] == nil then
			vRP.execute("vehicles/addVehicles",
				{ user_id = user_id, vehicle = vehName, plate = vRP.generatePlate(), work = "true" })
			...
			if vehSpawn[vehPlate] then
					...
					if os.time() >= parseInt(searchTimers[user_id]) then
						...
						if DoesEntityExist(idNetwork) and not IsPedAPlayer(idNetwork) and GetEntityType(idNetwork) == 2 then
							...
							TriggerClientEvent("Notify", source, "amarelo",
								"Rastreador do veículo foi ativado por <b>30 segundos</b>, lembrando que se o mesmo estiver em movimento a localização pode ser imprecisa.",
								10000)
						else
							if vehSpawn[vehPlate] then
							...
							TriggerClientEvent("Notify", source, "verde",
								"A seguradora efetuou o resgate do seu veículo e o mesmo já se encontra disponível para retirada.", 5000)
						end
					else
						TriggerClientEvent("Notify", source, "amarelo", "Rastreador só pode ser ativado a cada <b>60 segundos</b>.",
							5000)
					end
				else
				...
				if vehicle[1]["tax"] <= os.time() then
					TriggerClientEvent("Notify", source, "amarelo",
						"Taxa do veículo atrasada, efetue o pagamento<br>através do sistema da concessionária.", 5000)
				elseif vehicle[1]["arrest"] >= os.time() then
					TriggerClientEvent("Notify", source, "amarelo",
						"Veículo apreendido, dirija-se até o <b>Impound</b> e efetue o pagamento da liberação do mesmo.", 5000)
				else
					...
					if Coords then
						...
						if garageLocates[garageName]["payment"] then
							if vRP.userPremium(user_id) then
								TriggerClientEvent("dynamic:closeSystem", source)
								local netExist, netVeh, mHash = cRP.serverVehicle(vehName, Coords[1], Coords[2], Coords[3], Coords[4],
									vehPlate, vehicle[1]["nitro"], vehicle[1]["doors"], vehicle[1]["body"])

								if netExist then
									TriggerClientEvent("garages:createVehicle", source, mHash, netVeh, vehicle[1]["engine"], vehMods,
										vehicle[1]["windows"],
										vehicle[1]["tyres"])
								...
								if vRP.request(source, "Retirar o veículo por <b>$" .. parseFormat(vehPrice * 0.05) .. "</b> dólares?") then
									if vRP.getBank(user_id) >= parseInt(vehPrice * 0.05) then
										TriggerClientEvent("dynamic:closeSystem", source)
										local netExist, netVeh, mHash = cRP.serverVehicle(vehName, Coords[1], Coords[2], Coords[3], Coords
											[4], vehPlate, vehicle[1]["nitro"], vehicle[1]["doors"], vehicle[1]["body"])
...
RegisterCommand("car", function(source, args, rawCommand)
	...
	if user_id then
		if vRP.hasGroup(user_id, "Admin") or vRP.hasGroup(user_id, "Moderator") then
			...
			local netExist, netVeh, mHash, myVeh = cRP.serverVehicle(args[1], coords["x"], coords["y"], coords["z"], heading,
				vehPlate, 2000, nil, 1000)
			if not netExist then
				return
			end
			TriggerClientEvent("garages:createVehicle", source, mHash, netVeh, 1000, nil, false, false)
			vehSpawn[vehPlate] = { user_id, vehName, netVeh }
...
AddEventHandler("garages:Sell", function(vehModel)
	...
	if user_id then
		...
		if actived[user_id] == nil then
			...
			local sellText = "Vender o veículo <b>" .. vehicleName(vehModel) ..
					"</b> por <b>$" .. parseFormat(vehPrices) .. "</b>?"
		...
		if myVehicle[1] then
			...
			if identity then
				if vRP.request(source, "Transferir o veículo <b>" .. vehicleName(vehModel) .. "</b> para <b>" .. identity["name"] .. " " .. identity["name2"] .. "</b>?") then
					...
					if vehicle[1] then
						TriggerClientEvent("Notify", source, "amarelo",
							"<b>" .. identity["name"] .. " " .. identity["name2"] .. "</b> já possui este modelo de veículo.", 5000)
					else
						...
						if parseInt(#custom) > 0 then
							vRP.execute("entitydata/setData",
								{ dkey = "custom:" .. nuser_id .. ":" .. vehModel, value = custom[1]["dvalue"] })
