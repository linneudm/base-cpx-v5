		...
		if vRP.hasPermission(user_id, "Police") then
			...
			if identity then
				...
				TriggerClientEvent("Notify", source, "verde",
					...
					"</b> enviado para a prisão <b>" .. parseInt(services) .. " serviços</b>.", 5000)
			end
		end
	end
end)
		...
		if vRP.hasPermission(user_id, "Police") then
			...
			if identity then
				TriggerClientEvent("Notify", source, "azul",
					"<b>" ..
					identity.name ..
					" " .. identity.name2 .. "</b> teve sua pena reduzida em <b>" .. parseInt(services) .. "</b> serviços</b>.",
					5000)
			end
		end
	end
...
function cRP.reducePrison()
	...
	if user_id then
		...
		if parseInt(consult[1].prison) <= 0 then
			...
			TriggerClientEvent("Notify", source, "azul", "Você ainda tem <b>" .. parseInt(consult[1].prison) ..
				" serviços</b>.", 5000)
		end
	end
end
...
AddEventHandler("vRP:playerSpawn", function(user_id, source)
	...
	if parseInt(consult[1].prison) <= 0 then
		...
		TriggerClientEvent("Notify", source, "azul", "Você ainda tem <b>" .. parseInt(consult[1].prison) .. " serviços</b>.",
			5000)
		vCLIENT.startPrison(source, parseInt(consult[1].locate))
		...
		if vRP.hasPermission(user_id, "Police") then
			...
			if identity then
				...
				TriggerClientEvent("Notify", source, "amarelo",
					"Aplicado multa em <b>" ..
					identity.name .. " " .. identity.name2 .. "</b> no valor de <b>$" ..
					vRP.format(parseInt(value)) .. " dólares</b>.", 5000)
				creativeLogs(fines,
					"```PASSPORT: " ..
					parseInt(nuser_id) ..
					"\nNAME: " ..
					...
					identity.name2 ..
					"\nVALUE: $" ..
					vRP.format(parseInt(value)) .. "\nCRIMES: " .. reason .. "\nBY: " ..
					identity2.name .. " " .. identity2.name2 .. "```")
			end
		end
	end
end)
