	...
	if user_id then
		...
		if vehicle then
			...
			if userPlate then
				...
				return myInventory, myChest, vRP.inventoryWeight(user_id), vRP.getWeight(user_id), vRP.chestWeight(result),
						parseInt(vehWeight[user_id])
			end
		end
	end
end
...
function cRP.takeItem(slot, amount, target)
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		if not vRP.tryChest(user_id, vehChest[user_id], amount, slot, target) then
			TriggerClientEvent("trunkchest:Update", source, "updateMochila")
		end
	end
end
