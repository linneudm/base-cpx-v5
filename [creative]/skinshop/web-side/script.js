...
function ChangeUp() {
  var clothingCategory = $(changingCat)
    .parent()
    .parent()
    .parent()
    .parent()
...
$(document).on("change", ".item-number", function () {
  ...
  if (clothingCategory == "accessory" && inputVal == 13) {
    ...
    $.post(
      "http://skinshop/updateSkinOnInput",
      JSON.stringify({
        clothingType: clothingCategory,
  ...
  $.each(maxValues, function (i, cat) {
    if (cat.type == "character") {
      ...
      if (containers.data("type") != undefined) {
        ...
        $(itemMax).html(
          "<p><b>Modelos:</b> " +
            maxValues[containers.data("type")].item +
            "</p>",
        );
        $(headerMax).html(
          "<p><b>Texturas:</b> " +
